 ...
 * @author ArwaBj
 */
public class ServiceEnteteFacture implements IServiceEnteteFacture<Entete_facture>{
    Connection cnx = DataSource.getInstance().getCnx();

    @Override
    public void ajouter(Entete_facture t) {
         try {
            String requete1 = "INSERT INTO entete_facture (type ,date_exp ,tier) VALUES (?,?,?)";
...
try {
            String requete = "DELETE FROM entete_facture WHERE num_piece=?";
    ...
    public void modifier(Entete_facture t) {
        try {
            String requete = "UPDATE entete_facture SET type=?,date_exp=?,tier=? WHERE num_piece=?";
            PreparedStatement pst = cnx.prepareStatement(requete);
  ...
  List<Entete_facture> list = new ArrayList<>();
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new Entete_facture(rs.getInt(1), rs.getString(2), rs.getDate(3),rs.getInt(4)));
            }
    ...
    public List<Entete_facture> rechercher(Date date_exp){
            ...
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                list.add(new Entete_facture(rs.getInt(1), rs.getString("type"), rs.getDate("date_exp"),rs.getInt("tier")));
            }
